...
module.exports = {
  sendCallback (msg, item) {
    ...
    switch (item) {
      case 'packaging':
      case 'bots':
        text = `Хотите заказать ${item === 'bots' ? 'композицию' : 'упаковка'}?\nВы можете выбрать повод, на который Вы желаете ${item === 'bots' ? 'её' : 'его'} подарить , отсортировать ${item === 'bots' ? 'композиции' : 'упаковка'} по стоимости, или посмотреть весь ассортимент`
        keyboard = {
          inline_keyboard: [
            [{text: `🎉 Выбрать повод`, callback_data: item === 'bots' ? 'c_reasons' : 'b_reasons'}],
            [{text: `💰 Выбрать цену`, callback_data: item === 'bots' ? 'c_price' : 'b_price'}],
            [{text: `🔍 Смотреть все`, callback_data: item === 'bots' ? 'c_all' : 'b_all'}]
          ]
        }
        break
      case 'ads':
  ...
  showReasons (id, query) {
    ...
    return bot.sendMessage(id, `Выберите повод, на который хотите подарить ${item === 'b' ? 'упаковка' : 'композицию'}:`, {
